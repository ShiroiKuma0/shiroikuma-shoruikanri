/*
 * 白い熊 fork (skui): who is allowed through the automation data door, and how
 * that is decided. Copied from 自由作業盤's core/automation/AutomationCallers.kt
 * (contract v2, §2a) — deliberately self-contained and app-independent, so the
 * only fork-local part is the package it lives in.
 */
...
object SkAutomationCallers {
     ...
     * trust (shoruikanri, 2026-09-04) — see [HOW_TO_DERIVE_PINS].
     */
    ...
    fun verify(context: Context, declared: String?): Verdict {
        ...
        if (name !in real) {
            return Verdict.Refused("ERROR:caller uid mismatch: $name")
        }
     ...
     */
    private fun signingSha256(context: Context, packageName: String): String? =
        runCatching {
            val packageManager = context.packageManager
            // `signingInfo` and GET_SIGNING_CERTIFICATES are API 28, and this app's minSdk is 23.
            // On a 23–27 device the flag is accepted and `signingInfo` comes back null, so WITHOUT
            // this branch the door would refuse every caller — a total failure that never appears
            // on 白い熊's phone (API 31) and would only surface on an older one. The deprecated
            // array is the correct answer there, not a compromise: before key rotation existed,
            // `signatures` WAS the signing certificate.
            val certificates: Array<out Signature>? =
            ...
            // Exactly one signer, or we decline to guess. "Several signers, one of which matches"
            // is a question about key rotation that nothing in this family needs to answer — every
            // app here has one key and has never rotated it.
            val only = certificates?.singleOrNull() ?: return null
